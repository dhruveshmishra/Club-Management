        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="flex flex-wrap gap-4 items-center justify-between bg-card p-4 rounded-2xl border border-border shadow-sm">
            ...
            <div className="flex items-center gap-1.5 bg-background border border-border p-1 rounded-xl">
              <Link href={`/student?club=${selectedClubId}&mode=all&search=${search}`} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${mode === 'all' ? 'bg-muted text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                All
              </Link>
              <Link href={`/student?club=${selectedClubId}&mode=online&search=${search}`} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${mode === 'online' ? 'bg-muted text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                Online
              </Link>
              <Link href={`/student?club=${selectedClubId}&mode=offline&search=${search}`} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${mode === 'offline' ? 'bg-muted text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                Offline
              </Link>
            </div>
          </div>
